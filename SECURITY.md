...
- `pwds` intentionally shows machine usernames and passwords in the `fzf` UI; avoid using it during screen sharing or recording.
...
- `pwds` 会有意在 `fzf` 界面明文展示机器账号和密码；屏幕共享或录屏时不要使用。
